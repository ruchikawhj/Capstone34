...
function preload() {
    ...
    zombieAnimation=loadAnimation("Walk (3).png");
}
...
function keyPressed() {
  ...
  function showZombies() {
    if (zombies.length > 0) {
      if (
        zombies.length < 4 &&
        zombies[zombies.length - 1].body.position.x < width - 300
      ) {
    ...
    var zombie = new Zombie (width, height - 60, 170, 170, -60, zombieAnimation);
    zombies.push(zombie);
  }
}
